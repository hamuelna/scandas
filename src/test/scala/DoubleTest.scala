...
class DoubleTest extends FlatSpec with Matchers {
  ...
  it should "plus two series/double correctly" in {
    ...
    (ts1 + ts3).toVector should be (Vector(0.55, 1.123, 1.121, 1.498))
  ...
  it should "minus two series/double correctly" in {
    ...
    (ts1 - ts3).toVector should be (Vector(-0.16599999999999998, 0.121, -0.24500000000000005, 0.07200000000000006))
    (ts3 - ts4).toVector should be (Vector())
  ...
  it should "multiply two series/double together" in {
    ...
    (ts1 * ts3).toVector should be (Vector(0.068736, 0.311622, 0.29915400000000003, 0.559705))
    (ts3 * ts4).toVector should be (Vector())
  ...
  it should "divide two series/double together" in {
    ...
    (ts1 / ts3).toVector should be (Vector(0.5363128491620112, 1.2415169660678642, 0.6412884333821376, 1.1009817671809257))
    (ts3 / ts4).toVector should be (Vector())
